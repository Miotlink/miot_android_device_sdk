...
public class VspOperation {
	...
	private static boolean procCommonRes(VspMessage vm) {
		...
		int vspCommonResReqcode = vp.getIntValue(VspDefine.CommonRes_reqCode_idx);
...
//				return procGetPuList(vm);
		...
		private boolean procEnforceLogout(VspMessage vm) {
			Log.e("procEnforceLogout","procEnforceLogout");
...
//		("HasAppLogin=" + HasAppLogin());
	...
	public static boolean LoginPu(String userName, String password) throws Exception{
		...
		if (GetCmsAddress()) {
			...
			if (vspCallback!=null){
				vspCallback.loginRes(-3, "Authentication failed","");
			}
			return false;
		}
		if (vspCallback!=null){
			vspCallback.loginRes(-2, "fail","");
		}
		Reset();
		return false;
	}
